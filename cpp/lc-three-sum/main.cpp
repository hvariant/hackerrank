...
public:
  ...
  // alternative approach: choose one element, e.g. a[i], and then do two-sum in the range a[i+1:] on target -a[i]
  std::vector<std::vector<int>> threeSum(const std::vector<int>& input) {
    if(input.size() < 3) return {};

    std::vector<int> nums = input;
    std::sort(nums.begin(), nums.end());

    const auto advance = [](const auto& nums, size_t l)
    {
      size_t i=l+1;
    ...
    while(l < nums.size()-2)
    {
      ...
      int target = -nums[l];
      {
        ...
        while(i < j)
        {
          ...
          {
            ret.push_back({nums[l], nums[i], nums[j]});
            i = advance(nums, i);
          }
        }
      }
...
TEST_CASE("example 2")
{
  auto output = Solution().threeSum({ 0,0,0,0,0,0,0,0,0});
  auto output_set = std::set<std::vector<int>>{ output.begin(), output.end() };
  auto expected_set = std::set<std::vector<int>>{{0,0,0}};
  CHECK(output_set == expected_set);
}

TEST_CASE("example 3")
{
  auto output = Solution().threeSum({});
  auto output_set = std::set<std::vector<int>>{ output.begin(), output.end() };
  auto expected_set = std::set<std::vector<int>>{};
  CHECK(output_set == expected_set);
}

TEST_CASE("example TLE")
{
  std::vector<int> input = {82597,
                            -9243,
                            62390,
                            83030,
                            -97960,
